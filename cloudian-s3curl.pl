...
use Env qw(S3_ENDPOINT);

# begin customizing here
my @endpoints = ( 's3-sa-east-1.amazonaws.com', );

my $hostname=`/bin/hostname`;
push @endpoints, $hostname;

push @endpoints, $S3_ENDPOINT if(! $S3_ENDPOINT eq "");
...
Usage $0 --id friendly-name (or AWSAccessKeyId) [options] -- [curl-options] [URL]
 options:
  ...
  --createfolder	      PUT request, with /dev/null, remove content-type
  --amzstyledate	      Use x-amz-date instead Date header when signing
  --endpoint <endpoint>	      Add this endpoint to the list temporarily to avoid signing problems.
 common curl options:
...
push @endpoints,$endp  if(! $endp eq "");

my $method = "";
if (defined $fileToPut or defined $createBucket or defined $copySourceObject or defined $createFolder) {
    $method = "PUT";
...
my $xamzdate=POSIX::strftime("%a, %d %b %Y %H:%M:%S GMT", gmtime );
$xamzHeaders{'x-amz-date'}="$xamzdate" if(defined $amzstyledate);
...
my $stringToSign="";
if(defined $amzstyledate) {
	$stringToSign = "$method\n$contentMD5\n$contentType\n\n$xamzHeadersToSign$resource";
} else {
	$stringToSign = "$method\n$contentMD5\n$contentType\n$httpDate\n$xamzHeadersToSign$resource";
}
#print STDERR "StringToSign=-->'" . $stringToSign . "'<--" if(defined $debug);
...
my @args = ();
push @args, ("-H", "Date: $httpDate") if(!defined $amzstyledate);
push @args, ("-H", "Authorization: AWS $keyId:$signature");

push @args, ("-H", "x-amz-acl: $acl") if (defined $acl);
push @args, ("-L");
push @args, ("-H", "content-type: $contentType") if (defined $contentType  && !defined $createFolder);
push @args, ("-H", "Content-MD5: $contentMD5") if (length $contentMD5);
push @args, ("-H", "content-length: 0") if (defined $createFolder);
...
push @args, ("-H", "x-amz-date: $xamzdate") if(defined $amzstyledate);
...
#if(!defined($selfcheck)) {
	exec($CURL, @args)  or die "can't exec program: $!";
#} else {
#   	print STDERR "curl @args\n";
#}
